...
/// One MiB. The Server Process declares the same ceiling in its own
/// `graphql_route`; `scripts/cross-language-constants.test.mjs` holds the two
/// in agreement so the Targets cannot drift to different limits.
...
pub fn error_response(message: &str, code: &str, detail: &str) -> String {
    serde_json::json!({
        "data": null,
        "errors": [{
            "message": message,
...
pub fn service_unavailable_response() -> String {
    error_response(
